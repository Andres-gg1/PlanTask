...
def history_page(request):
    ...
    if role == 'admin':
        ...
        actions = [serialize_activity_log(action) for action in actions]  # Serialize objects
    else:
        actions = []

    my_actions = request.dbsession.query(ActivityLog).filter_by(user_id=user_id).all()
    my_actions = [serialize_activity_log(action) for action in my_actions]  # Serialize objects
